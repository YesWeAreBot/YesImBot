...
import { jsonSchema as aiJsonSchema } from "@ai-sdk/provider-utils";
import type { JSONSchema4 } from "json-schema";
import { Schema } from "koishi";

import { MixedInputSchema } from "./tools/types";
...
export function normalizeInputSchema(schema: MixedInputSchema): FlexibleSchema<unknown> {
  if (isKoishiSchema(schema)) {
    const json = schemaToJSONSchema(schema);
    return aiJsonSchema(json as Parameters<typeof aiJsonSchema>[0]);
  }

  if (isJsonSchema(schema)) {
    return aiJsonSchema(schema as Parameters<typeof aiJsonSchema>[0]);
  }

  return schema;
}

export function isKoishiSchema(value: unknown): value is Schema {
  if (!value || typeof value !== "object") return false;
  const record = value as Record<string, unknown>;
  return (
    typeof record.type === "string" && ("meta" in record || "dict" in record || "inner" in record)
  );
}

export function isJsonSchema(value: unknown): value is JSONSchema4 {
  if (!value || typeof value !== "object") return false;
  const record = value as Record<string, unknown>;
