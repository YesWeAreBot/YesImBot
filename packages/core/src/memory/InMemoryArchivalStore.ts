...
export class InMemoryArchivalStore implements IArchivalMemoryStore {
    ...
    async search(query: string, options: { page?: number; pageSize?: number, filterMetadata?: Record<string, any> } = {}): Promise<ArchivalSearchResult> {
        ...
        let matched = Array.from(this.entries.values()).filter(entry => {
            ...
            if (filterMetadata) {
                metadataMatch = Object.entries(filterMetadata).every(([key, value]) =>
                    entry.metadata && String(entry.metadata[key]).toLowerCase() === String(value).toLowerCase()
                );
            }
