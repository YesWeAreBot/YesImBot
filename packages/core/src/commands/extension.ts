...
// 获取扩展目录路径
function getExtensionPath(ctx: Context, builtin: boolean = false): string {
    const isDevMode = process.env.NODE_ENV === 'development'
    if (builtin) {
        return path.join(
            ctx.baseDir,
...
async function getExtensionFiles(ctx: Context): Promise<string[]> {
    ...
    try {
        const builtinFiles = (await fs.readdir(builtin, { recursive: true }).catch(() => []))?.map(file => path.join(builtin, file)) || [];
        const userFiles = (await fs.readdir(user).catch(() => []))?.map(file => path.join(builtin, file)) || [];
        const files = Array.from(new Set([...builtinFiles, ...userFiles]));
        return files.filter(file => {
            file = path.basename(file);
            return file.startsWith('ext_') &&
                file.endsWith('.js') &&
                !file.endsWith('.map') &&
                !file.endsWith('.d.js')
        });
