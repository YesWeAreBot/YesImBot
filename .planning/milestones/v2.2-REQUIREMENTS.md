# Requirements Archive: v2.2 Runtime Optimization & Observability
...
- [x] **OBS-01**: 每条消息处理流程携带 traceId，贯穿 listener → willingness → agent → loop → model → parser → reply 全链路
- [x] **OBS-02**: 使用 Koishi Logger 命名空间（`agent`、`agent.willingness`、`agent.loop`、`agent.parser` 等），支持 `KOISHI_DEBUG` 环境变量粒度过滤
- [x] **OBS-03**: 关键节点输出 debug 级别结构化日志：意愿值决策详情、prompt section 大小、模型调用延迟/token 用量、JSON 解析结果、工具执行结果
