...
- [x] **BUGFIX-02**: JSON Parser 拥有完整的 vitest 测试套件，覆盖 v3 的 18 个测试用例（完美 JSON、代码块、嵌套代码块、`[OBSERVE]` 前缀、截断字符串、悬空键等）
...
- [x] **OPT-01**: System prompt 拆分为 `SystemModelMessage[]` content blocks，稳定部分（soul + instructions）标记 cache breakpoint
- [x] **OPT-02**: ModelService 支持 provider 检测，Anthropic 自动注入 `providerOptions` cache control，其他 provider 回退为字符串拼接
...
| OPT-01 | Phase 25 | Complete |
| OPT-02 | Phase 25 | Complete |
