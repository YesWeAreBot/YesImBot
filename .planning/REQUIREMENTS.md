...
- [ ] **WILL-01**: 私聊消息自动获得高回复概率（可配置 `directBoost`），不再走群聊相同的概率判定
- [ ] **WILL-02**: 私聊回复附带 per-user 速率限制，防止无节制 DM 导致成本爆炸
...
- [ ] **OPT-03**: Working Memory 按时间正序排列（最早 round 在前），消除逆序导致的 LLM 时间推理混乱
