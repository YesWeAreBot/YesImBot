...
export class ModelService extends Service<ModelServiceConfig> implements IModelService {
  ...
  private async handleFallback(primaryProvider: string, primaryModel: string, params: any, error: unknown) {
  ...
  private async handleStreamFallback(primaryProvider: string, primaryModel: string, params: any, error: unknown) {
    const key = `${primaryProvider}:${primaryModel}`
    const chain = this.config.fallbackChains?.[key]
