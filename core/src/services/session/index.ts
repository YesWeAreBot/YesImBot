...
export class SessionService extends Service {
  ...
  protected async start() {
    ...
    this.ctx.command("new", "创建新会话").action(async ({ session }) => {
      ...
      const newManager = await this.newSession(session.platform, session.channelId);
  ...
  private getChannelDir(platform: string, channel: string): string {
    const encoded = encodeChannelId(platform, channel);
    return join(this.basePath, "sessions", encoded);
  }

  async getOrCreate(platform: string, channel: string): Promise<SessionManager> {
    ...
    if (meta?.current_session) {
      ...
      if (existsSync(sessionFile)) {
        ...
        writeMeta(channelDir, createMeta(platform, channel, sessionManager.getSessionFile()!, 1));
      }
    } else {
      sessionManager = SessionManager.create(this.basePath, channelDir);
      writeMeta(channelDir, createMeta(platform, channel, sessionManager.getSessionFile()!, 1));
  ...
  async newSession(platform: string, channel: string): Promise<SessionManager> {
    const key: ChannelKey = `${platform}:${channel}`;
    ...
    writeMeta(
      channelDir,
      createMeta(platform, channel, sessionManager.getSessionFile()!, sessionCount),
    );
