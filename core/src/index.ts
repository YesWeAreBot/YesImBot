...
export const Config = Schema.object({
  ...
  runtimeSettings: Schema.object({
    contextWindow: Schema.number().min(1000).max(1000000).default(128000).description("上下文窗口大小（token 数）"),
    compaction: Schema.object({
      enabled: Schema.boolean().default(true).description("启用上下文压缩"),
      reserveTokens: Schema.number().min(1000).max(100000).default(16384).description("压缩保留的 token 数"),
      keepRecentTokens: Schema.number().min(1000).max(100000).default(20000).description("保留最近消息的 token 数"),
    }).description("上下文压缩配置"),
    retry: Schema.object({
      enabled: Schema.boolean().default(true).description("启用自动重试"),
      maxRetries: Schema.number().min(0).max(10).default(3).description("最大重试次数"),
      baseDelayMs: Schema.number().min(100).max(30000).default(2000).description("基础重试延迟（毫秒）"),
      maxDelayMs: Schema.number().min(1000).max(300000).default(60000).description("最大重试延迟（毫秒）"),
    }).description("重试配置"),
    steeringMode: Schema.union(["all", "one-at-a-time"] as const).default("all").description("引导模式"),
    followUpMode: Schema.union(["all", "one-at-a-time"] as const).default("all").description("跟进模式"),
    delivery: Schema.object({
      enabled: Schema.boolean().default(true).description("启用消息分段发送"),
      segmentation: Schema.object({
        shortSegmentChars: Schema.number().min(1).max(100).default(6).description("短段落字符数阈值"),
        shortTextChars: Schema.number().min(1).max(500).default(25).description("短文本字符数阈值"),
      }).description("分段配置"),
      timing: Schema.object({
        initialDelayMinMs: Schema.number().min(0).max(10000).default(300).description("初始延迟最小值（毫秒）"),
        initialDelayMaxMs: Schema.number().min(0).max(10000).default(1200).description("初始延迟最大值（毫秒）"),
        followupDelayMinMs: Schema.number().min(0).max(30000).default(1200).description("后续延迟最小值（毫秒）"),
        followupDelayMaxMs: Schema.number().min(0).max(30000).default(4500).description("后续延迟最大值（毫秒）"),
        maxDelayMs: Schema.number().min(0).max(60000).default(6500).description("最大延迟（毫秒）"),
        minimumBufferMinMs: Schema.number().min(0).max(5000).default(150).description("最小缓冲区最小值（毫秒）"),
        minimumBufferMaxMs: Schema.number().min(0).max(5000).default(400).description("最小缓冲区最大值（毫秒）"),
      }).description("延迟配置"),
