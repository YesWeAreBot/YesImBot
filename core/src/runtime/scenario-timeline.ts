...
import { TimelineEventType } from "../services/horizon/types";
import {
          ...
          if (isSuccessfulSendMessage(result)) {
            const sendResult =
              result.result && typeof result.result === "object"
                ...
                : undefined;
            const topLevelMessageId = readString(sendResult?.messageId);
            const topLevelContent = readString(sendResult?.content);
            const firstMessage = readFirstMessage(sendResult?.messages);
            currentTurn.visibleOutputs.push({
              toolName: "send_message",
              success: true,
              messageId: topLevelMessageId ?? readString(firstMessage?.messageId),
              content: topLevelContent ?? readString(firstMessage?.content),
              timestamp,
            });
          }
        }
...
function toHeartbeat(entry: TimelineEntry, timestamp: Date): ScenarioTimelineHeartbeat {
  ...
  const detail: Record<string, unknown> = {
    channelSummary:
      typeof data.channelSummary === "string" && data.channelSummary.trim().length > 0
        ? data.channelSummary
        : "Periodic check-in",
  };
  return {
...
function readFirstMessage(value: unknown): Record<string, unknown> | undefined {
